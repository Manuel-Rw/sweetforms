...
import { requiredValidator, emailValidator, confirmPasswordValidator, customValidator, fileValidator } from "../validators/index.js"
...
export const formDidRender = ({fields}) => {
    ...
    inputs.forEach(input => {
        let field = fields.find(field => input.id.includes(`--swInput${field.key}--`))
        ...
        if(field.type == 'password' && field.confirmPassword) {
            const confirmPassInput = document.getElementById(`--swInput${field.key}--confirm--`)
            confirmPassInput.addEventListener("focusin", () => confirmPassInput.classList.add('swFocus'))
            confirmPassInput.addEventListener("focusout", () => confirmPassInput.classList.remove('swFocus'))
        }
    })
}
    ...
    inputs.forEach(input => {
        ...
        if (['date', 'datetime-local' , 'time', 'month', 'week', 'file', 'checkbox', 'radio', 'select'].includes(field.type)) {
            ...
            input.addEventListener("keyup", async () => {
                ...
                if(field.customValidation && customValidators) customValidator({field, customValidators})
                if(field.type == 'password' && field.confirmPassword) confirmPasswordValidator({field})
            })
        }

        if(field.type == 'password' && field.confirmPassword) {
            document.getElementById(`--swInput${field.key}--confirm--`).addEventListener('keyup', async => {
...
export const formPreCheck = async ({ fields, customValidators }) => {
    return Promise.all(fields.map(field => {
        return new Promise(async (resolve) => {
            ...
            if(field.type == 'password' && field.confirmPassword) confirmPasswordValidator({field, resolve})
